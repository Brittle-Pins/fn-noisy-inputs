...
'''
    ...
    with open(header_path, 'w', encoding='utf-8') as f:
        f.write(header)
    print(f'Wrote TFLite C header: {header_path}')

    # --- Export raw weights for the manual forward pass in mlp_inference.cpp ---
    # This avoids requiring the TFLite Micro runtime on the device while still using
    # TFLiteConverter above as the authoritative model serialisation step.

    norm_layer = next(l for l in model.layers if isinstance(l, tf.keras.layers.Normalization))
    dense_layers = [l for l in model.layers if isinstance(l, tf.keras.layers.Dense)]

    norm_mean = norm_layer.mean.numpy().flatten()
    # Normalization computes (x - mean) / sqrt(variance + epsilon); epsilon = 1e-3 by default.
    norm_std = np.sqrt(norm_layer.variance.numpy().flatten() + 1e-3)

    # Keras stores Dense weights as (in, out).  Transpose to (out, in) so the C forward
    # pass can index as W[j * IN + i] for output neuron j and input i.
    W1, b1 = dense_layers[0].get_weights()   # (28, 32), (32,)
    W2, b2 = dense_layers[1].get_weights()   # (32, 16), (16,)
    W3, b3 = dense_layers[2].get_weights()   # (16,  1), ( 1,)

    def arr_to_c(name, arr, cols=8):
        flat = arr.flatten()
        entries = [f'{v:.8f}f' for v in flat]
        rows = [
            '    ' + ', '.join(entries[i:i + cols])
...
// Auto-generated by mlp.py — do not edit by hand.
// Architecture: Normalization -> Dense({W1.shape[1]}, relu) -> Dense({W2.shape[1]}, relu) -> Dense(1, sigmoid)

#include <stdint.h>

#define MLP_INPUT {W1.shape[0]}
#define MLP_H1    {W1.shape[1]}
#define MLP_H2    {W2.shape[1]}

// Per-feature normalisation: x_norm[i] = (x[i] - mean[i]) / std[i]
{arr_to_c('mlp_norm_mean', norm_mean)}

{arr_to_c('mlp_norm_std', norm_std)}

// Dense layer 1 weights, shape (H1 x INPUT), row-major
{arr_to_c('mlp_w1', W1.T)}

{arr_to_c('mlp_b1', b1)}

// Dense layer 2 weights, shape (H2 x H1), row-major
{arr_to_c('mlp_w2', W2.T)}

{arr_to_c('mlp_b2', b2)}

// Output layer weights, shape (H2,), and scalar bias
{arr_to_c('mlp_w3', W3.flatten())}

static const float mlp_b3 = {float(b3[0]):.8f}f;
'''

    weights_path = os.path.join('include', 'mlp_weights.h')
    with open(weights_path, 'w', encoding='utf-8') as f:
        f.write(weights_header)
    print(f'Wrote weights header: {weights_path}')
